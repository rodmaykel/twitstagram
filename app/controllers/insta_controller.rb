class InstaController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @instum.save
        ...
        format.json { render action: 'show', status: :created, location: @instum }
      else
  ...
  def destroy
    @instum.destroy
    respond_to do |format|
      format.html { redirect_to insta_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def instum_params
      params.require(:instum).permit(:subject_id, :created, :photo, :caption)
    end
end
